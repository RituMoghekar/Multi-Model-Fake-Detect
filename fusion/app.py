...
from predict_text import predict_text  # zero-shot text predictor
...
def result_card(modality: str, label: str, confidence: float, explanation: str):
    ...
    explain_html = f'<div class="explain-box">ℹ️ {explanation}</div>' if explanation and explanation != "N/A" else ""
        ...
        with st.spinner("⚡ Analyzing content... Please wait"):
            ...
            if input_type == "Audio" and uploaded_file:
                ...
                try:
                    ...
                    # 🧠 Whisper Transcription Display (AFTER prediction)
                    st.markdown('<div class="section-label"> EXTRACTED SPEECH</div>', unsafe_allow_html=True)

                    transcription = audio_result.get("transcription", "No transcription available")
            ...
            if input_type == "Video" and uploaded_file:
                ...
                try:
                    ...
                    for r in video_results:
                        result_card(r['modality'], r['label'], r['confidence'], r.get("explanation", "N/A"))
                except Exception as e:
            ...
            if input_type == "Text" and uploaded_text and uploaded_text.strip():
                ...
                try:
                    ...
                    result_card("Text", text_result['label'], text_result['confidence'], text_result.get("explanation", "N/A"))
                except Exception as e:
            ...
            if len(predictions) >= 2:
                ...
                for detail in fusion_result["details"]:
                    formatted = html.escape(json.dumps(detail, indent=2))
                    
                    details_html += f"""<div class="detail-json" style="
		    margin:6px 0;
		    ...
		    padding:10px;
		    ">
		<pre style="margin:0; white-space: pre-wrap; word-break: break-word;">{formatted}</pre>
		</div>"""
                ...
                <div class="fusion-panel">
                  ...
                  <div class="conf-bar-wrap" style="height:8px; margin-bottom:1rem">
                    <div class="conf-bar" style="width:{fs_pct}%; background: linear-gradient(90deg, {fcolor}66, {fcolor});"></div>
                  </div>
